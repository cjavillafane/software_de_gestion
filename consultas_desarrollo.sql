...
,personas_idpersona,periodos_idperiodo) VALUES(1,'07/03/2022','1234-5','213213213_CAE','08/03/2022',12000.00,10000.00,500.00
,'Retencion por exportación',700.43,1,800.76,2354.53,856.87,'Retencion por metal',1,9649.99,677.97,9743.65
,723.94,10500.25,0.00,now(),'comprobante de compra','compra de mercaderia','A',7,1,1);
...
,diferencia_con_total,fecha,empresas_idempresa,personas_idpersona,periodos_idperiodo) 
...
,1,'Retencion por metal pulido','TUERCAS PULIDAS',0.00,12.98,34.98,234.7,null,0,now(),7,2,1);
...
SELECT * FROM compras c INNER JOIN personas p ON c.personas_idpersona=p.idpersona
	INNER JOIN periodos per ON c.periodos_idperiodo=per.idperiodo
          WHERE c.empresas_idempresa=7 AND c.periodos_idperiodo=1;
